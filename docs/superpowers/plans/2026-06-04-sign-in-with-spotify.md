...
Create `test/spotify-provider-connect.test.js`:

```js
const assert = require('assert');
const Database = require('better-sqlite3');

// Inject an in-memory settings DB in place of db/init.getDb() BEFORE requiring
// spotify.js (which destructures getDb at module load).
...
const get = (k) => mem.prepare('SELECT value FROM settings WHERE key = ?').get(k)?.value;

(function run() {
  // A stale playlist id from a previous account must be cleared on connect.
  mem.prepare('INSERT INTO settings (key, value) VALUES (?, ?)').run('spotify_playlist_id', 'OLD');
  ...
  assert.strictEqual(get('spotify_playlist_id'), undefined, 'stale playlist id cleared');
  console.log('✓ persists tokens and clears stale playlist id');
